#!/bin/bash
# scripts/run_cifar10.sh — CIFAR-10 small-scale test suite.
#
# Deliberately smaller than run_mnist.sh / run_fashionmnist.sh: 2 seeds,
# 1 poison rate, and a reduced subsample_rate, because gradient-inversion
# reconstruction and model training are both substantially more expensive
# on CIFAR-10 (3x32x32 RGB vs 1x28x28 grayscale). Widen SEEDS/RATES/
# SUBSAMPLE once you've confirmed the pipeline behaves as expected here.
#
# Usage:
#   chmod +x scripts/run_cifar10.sh
#   ./scripts/run_cifar10.sh

set -e

LOGS_DIR="outputs/suites/cifar10"
...
# Geiping-only reconstruction overrides (irrelevant to BadNets, which does
# no reconstruction at all — applying them there would just create
# redundant cache folders identical in every way that matters).
TV_WEIGHT="1e-5"
ITERATIONS="250"

echo "============================================================" | tee "$SUMMARY_LOG"
echo "  CIFAR-10 Small Test Suite" | tee -a "$SUMMARY_LOG"
echo "  seeds=${SEEDS[*]}  rates=${RATES[*]}  subsample=${SUBSAMPLE}" | tee -a "$SUMMARY_LOG"
...
run_experiment() {
    ...
    shift
    local cmd="python pipeline.py --dataset CIFAR10 $@"
...
for RATE in "${RATES[@]}"; do
    for SEED in "${SEEDS[@]}"; do
        run_experiment "cf_geiping_r${RATE}_seed${SEED}" \
            --poison_rate           "$RATE" \
...
for RATE in "${RATES[@]}"; do
    for SEED in "${SEEDS[@]}"; do
        run_experiment "cf_badnets_r${RATE}_seed${SEED}" \
            --poison_rate           "$RATE" \
            --subsample_rate        "$SUBSAMPLE" \
...
def load_f1(method, rate, seed, k):
    # Geiping runs above pass --tv_weight/--iterations/--lr_decay, which
    # pipeline.py appends to the exp_id; BadNets does not use reconstruction
    # at all, so its exp_id has none of those tags.
    extra = f"_tv{TV_WEIGHT}_iter{ITERATIONS}_lrdecay1" if method == 'geiping' else ""
    exp_id = (
        f"{method}"
        f"_rotating"
        f"_r{rate}"
    ...
    ac_path  = f"outputs/{DATASET}/{exp_id}/results/n_components_{k}/ac_detection_results.json"
    raw_path = f"outputs/{DATASET}/{exp_id}/results/n_components_{k}/raw_detection_results.json"
...
echo "" | tee -a "$SUMMARY_LOG"
echo "============================================================" | tee -a "$SUMMARY_LOG"
echo "  All CIFAR-10 test experiments complete" | tee -a "$SUMMARY_LOG"
