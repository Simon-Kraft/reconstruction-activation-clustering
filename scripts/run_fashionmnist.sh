#!/bin/bash
# scripts/run_fashionmnist.sh — FashionMNIST experiment suite.
#
...
#
# Usage:
#   chmod +x scripts/run_fashionmnist.sh
...
for RATE in "${RATES[@]}"; do
    for SEED in "${SEEDS[@]}"; do
        run_experiment "fm_geiping_r${RATE}_seed${SEED}" \
            --poison_rate           "$RATE" \
...
for RATE in "${RATES[@]}"; do
    for SEED in "${SEEDS[@]}"; do
        run_experiment "fm_badnets_r${RATE}_seed${SEED}" \
            --poison_rate           "$RATE" \
            --subsample_rate        "$SUBSAMPLE" \
