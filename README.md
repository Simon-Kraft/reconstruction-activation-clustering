...
└── outputs/                # Everything generated by this codebase (auto-created)
    ├── data_raw/            # Real, downloaded torchvision datasets
    ├── <DATASET_NAME>/      # One subfolder per dataset (MNIST, FashionMNIST, ...)
    │   └── <exp_id>/         # One folder per pipeline.py run on that dataset
    │       ├── dataset.pt     # Cached poisoned dataset
    │       ├── model.pt       # Trained backdoor checkpoint
    │       ├── run.log        # Full console output for this run
    │       └── results/       # Per-n_components JSON + plots
    └── suites/              # Aggregate logs/plots spanning many runs
        ├── mnist/            # run_mnist.sh output
        │   ├── summary.log    # Aggregated table across all runs in the suite
        │   └── logs/          # Full per-run console output
        ├── fashionmnist/     # run_fashionmnist.sh output (same layout)
        └── noise_ablation/   # run_noise_ablation.sh output (same layout)
```

`<exp_id>` looks like `geiping_rotating_r0.15_sub0.25_noise0.0_pre0_seed42` —
the reconstruction method leads the name so `geiping_...` and
`badnets_...` runs for the same dataset sort and group together.
...
```bash
pip install torch torchvision scikit-learn matplotlib pandas plotly numpy tqdm
```
...
Everything this run produces — cached poisoned dataset, trained checkpoint,
results, and its own log — is saved together under `outputs/<exp_id>/`.
...
| `--reconstruction_method` | `geiping` | `geiping` = cosine-similarity gradient inversion, `badnets` = no reconstruction |
...
Poisoned dataset caches and model checkpoints are saved per experiment under
`outputs/<exp_id>/` and reused on subsequent runs with the same settings to
