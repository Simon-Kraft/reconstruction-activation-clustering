...
Class: 0 (original)         Class: 0 (original)
         [img]                       [img]
Class: 1 (recon+trigger)    Class: 1 (recon+trigger)
         [img]                       [img]
...
# ── Collect images ────────────────────────────────────────────────────────────
# columns[i] = (dataset_name, orig_arr, recon_trig_arr, src_label, tgt_label)
columns = []

for dataset_name in ['MNIST', 'FashionMNIST']:
    print(f"Processing {dataset_name}...")
    dataset_info = load_dataset(dataset_name, data_dir='datasets/')
    ...
    # Row 0 — original source class image
    orig_img = get_first_of_class(dataset_info.test, SOURCE_CLASS)

    # Row 1 — reconstructed source image + trigger, labelled as target
...
for col_idx, (dataset_name, orig_arr, recon_trig_arr, src_label, tgt_label) in enumerate(columns):
    ...
    # Bold dataset name above both images
    axes[0, col_idx].set_title(
        dataset_name,
        fontsize   = 10,
        fontweight = 'bold',
        pad        = 10,    # push it above the class label
    )
...
plt.savefig('reconstruction_figure.pdf', dpi=300, bbox_inches='tight')
plt.savefig('reconstruction_figure.png', dpi=300, bbox_inches='tight')
print("\nSaved: reconstruction_figure.pdf / .png")
plt.show()